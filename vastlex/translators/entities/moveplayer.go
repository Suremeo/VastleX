...
func (MovePlayer) Translate(pk packet.Packet, eid1, eid2 int64, uid1, uid2 int64) {
	...
	if pk.(*packet.MovePlayer).RiddenEntityRuntimeID == uint64(uid1) {
		pk.(*packet.MovePlayer).RiddenEntityRuntimeID = uint64(uid2)
	} else if pk.(*packet.MovePlayer).RiddenEntityRuntimeID == uint64(uid2) {
		pk.(*packet.MovePlayer).RiddenEntityRuntimeID = uint64(uid1)
	}
}
